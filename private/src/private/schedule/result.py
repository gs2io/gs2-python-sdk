...
class DescribeTriggersByOwnerIdAndUserIdResult(Gs2Result):
    """
    オーナーIDを指定してトリガーの一覧を取得 のレスポンスモデル
    """
    ...
    def __init__(
        ...
        super(DescribeTriggersByOwnerIdAndUserIdResult, self).__init__()
        # トリガーのリスト
        self._items = []
        for item in response.get('items', []):
            self._items.append(model.Trigger(item))
        # リストの続きを取得するためのページトークン
        self._next_page_token = response.get('nextPageToken') if 'nextPageToken' in response else None

    @property
    def items(self) -> List[model.Trigger]:
        return self._items
    ...
    def __getitem__(self, key):
        ...
        if key in items.keys():
            return items[key]
        return super(DescribeTriggersByOwnerIdAndUserIdResult, self).__getitem__(key)
...
class GetEventByOwnerIdAndUserIdResult(Gs2Result):
    """
    オーナーIDとユーザIDを指定してイベントを取得 のレスポンスモデル
    """
    ...
    def __init__(
        ...
        super(GetEventByOwnerIdAndUserIdResult, self).__init__()
        # イベント
        self._item = model.Event(response.get('item')) if 'item' in response else None

    @property
    def item(self) -> model.Event:
        return self._item
    ...
    def __getitem__(self, key):
        ...
        if key in items.keys():
            return items[key]
        return super(GetEventByOwnerIdAndUserIdResult, self).__getitem__(key)
