...
class CreateRoomResult(Gs2Result):
    """
    ルームを作成。 のレスポンスモデル
    """
    ...
    def __init__(
        ...
        super(CreateRoomResult, self).__init__()
        # ルーム
        self._item = model.Room(response.get('item')) if 'item' in response else None

    @property
    def item(self) -> model.Room:
        return self._item
...
class ShutdownRoomResult(Gs2Result):
    """
    ルームを停止 のレスポンスモデル
    """
    ...
    def __init__(
        ...
        super(ShutdownRoomResult, self).__init__()
    ...
    def __getitem__(self, key):
        ...
        if key in items.keys():
            return items[key]
        return super(ShutdownRoomResult, self).__getitem__(key)
