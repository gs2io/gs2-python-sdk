...
class DescribeNamespacesByOwnerIdResult(Gs2Result):
    """
    オーナーIDを指定してネームスペースの一覧を取得します のレスポンスモデル
    """
...
class DoneGenerateResult(Gs2Result):
    """
    現在有効なお知らせの完了を報告 のレスポンスモデル
    """
    ...
    def __init__(
        ...
        super(DoneGenerateResult, self).__init__()
    ...
    def __getitem__(self, key):
        ...
        if key in items.keys():
            return items[key]
        return super(DoneGenerateResult, self).__getitem__(key)
