...
class GetCounterByOwnerIdAndUserIdResult(Gs2Result):
    """
    カウンターを取得 のレスポンスモデル
    """
    ...
    def __init__(
        ...
        super(GetCounterByOwnerIdAndUserIdResult, self).__init__()
        # カウンター
        self._item = model.Counter(response.get('item')) if 'item' in response else None

    @property
    def item(self) -> model.Counter:
        return self._item
    ...
    def __getitem__(self, key):
        ...
        if key in items.keys():
            return items[key]
        return super(GetCounterByOwnerIdAndUserIdResult, self).__getitem__(key)
