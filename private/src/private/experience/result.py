...
class DescribeStatusesByOwnerIdAndUserIdResult(Gs2Result):
    """
    オーナーIDを指定してステータスの一覧を取得 のレスポンスモデル
    """
    ...
    def __init__(
        ...
        super(DescribeStatusesByOwnerIdAndUserIdResult, self).__init__()
        # ステータスのリスト
        self._items = []
        for item in response.get('items', []):
            self._items.append(model.Status(item))
        # リストの続きを取得するためのページトークン
        self._next_page_token = response.get('nextPageToken') if 'nextPageToken' in response else None

    @property
    def items(self) -> List[model.Status]:
        return self._items
    ...
    def __getitem__(self, key):
        ...
        if key in items.keys():
            return items[key]
        return super(DescribeStatusesByOwnerIdAndUserIdResult, self).__getitem__(key)
