...
from script.web_socket import Gs2ScriptWebSocketClient
import private.script.request as request_model
import private.script.result as result_model


class Gs2ScriptPrivateWebSocketClient(Gs2ScriptWebSocketClient):
    ...
    def _invoke(
        self,
        request: request_model.InvokeRequest,
        callback: Callable[[AsyncResult[result_model.InvokeResult]], None],
    ):
        """
        スクリプトを実行します
        :param request: Request
        ...
        self.session.send(
            NetworkJob(
                request_id=request_id,
                result_type=result_model.InvokeResult,
                callback=callback,
                body=body,
            )
        )
