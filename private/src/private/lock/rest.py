...
class Gs2LockPrivateRestClient(Gs2LockRestClient):
    ...
    def _lock_by_owner_id_and_user_id(
        ...
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='lock',
            region=self.session.region,
        ) + "/system/{ownerId}/{namespaceName}/user/{userId}/mutex/{propertyId}/lock".format(
            ownerId=request.owner_id if request.owner_id is not None and request.owner_id != '' else 'null',
    ...
    def _unlock_by_owner_id_and_user_id(
        ...
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='lock',
            region=self.session.region,
        ) + "/system/{ownerId}/{namespaceName}/user/{userId}/mutex/{propertyId}/unlock".format(
            ownerId=request.owner_id if request.owner_id is not None and request.owner_id != '' else 'null',
            namespaceName=request.namespace_name if request.namespace_name is not None and request.namespace_name != '' else 'null',
            userId=request.user_id if request.user_id is not None and request.user_id != '' else 'null',
            propertyId=request.property_id if request.property_id is not None and request.property_id != '' else 'null',
    ...
    def _get_mutex_by_owner_id_and_user_id(
        ...
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='lock',
            region=self.session.region,
        ) + "/system/{ownerId}/{namespaceName}/user/{userId}/mutex/{propertyId}".format(
            ownerId=request.owner_id if request.owner_id is not None and request.owner_id != '' else 'null',
            namespaceName=request.namespace_name if request.namespace_name is not None and request.namespace_name != '' else 'null',
            userId=request.user_id if request.user_id is not None and request.user_id != '' else 'null',
            propertyId=request.property_id if request.property_id is not None and request.property_id != '' else 'null',
