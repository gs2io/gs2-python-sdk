...
from ranking import Gs2RankingRestClient, CategoryModel, request
from ranking.domain.cache.category_model import CategoryModelDomainCache
from core.domain.access_token import AccessToken


class DescribeCategoryModelsIterator:
    ...
    def _load(
        ...
        r = self._client.describe_category_models(
            request.DescribeCategoryModelsRequest()
                .with_namespace_name(self._namespace_name)
        )
        for item in r.items:
            self._category_model_cache.update(item)
    ...
    def __next__(self) -> CategoryModel:
