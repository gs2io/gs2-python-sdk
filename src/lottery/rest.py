...
class Gs2LotteryRestClient(AbstractGs2RestClient):
    ...
    def _get_raw_box_by_user_id(
        self,
        request: GetRawBoxByUserIdRequest,
        callback: Callable[[AsyncResult[GetRawBoxByUserIdResult]], None],
        is_blocking: bool,
    ):
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='lottery',
            region=self.session.region,
        ) + "/{namespaceName}/user/{userId}/box/{prizeTableName}/raw".format(
            namespaceName=request.namespace_name if request.namespace_name is not None and request.namespace_name != '' else 'null',
            prizeTableName=request.prize_table_name if request.prize_table_name is not None and request.prize_table_name != '' else 'null',
            userId=request.user_id if request.user_id is not None and request.user_id != '' else 'null',
        )
        ...
        _job = NetworkJob(
            url=url,
            method='GET',
            result_type=GetRawBoxByUserIdResult,
            callback=callback,
            headers=headers,
            query_strings=query_strings,
        )
    ...
    def get_raw_box_by_user_id(
        self,
        request: GetRawBoxByUserIdRequest,
    ) -> GetRawBoxByUserIdResult:
        async_result = []
        with timeout(30):
            self._get_raw_box_by_user_id(
                request,
                lambda result: async_result.append(result),
                is_blocking=True,
            )
    ...
    async def get_raw_box_by_user_id_async(
        self,
        request: GetRawBoxByUserIdRequest,
    ) -> GetRawBoxByUserIdResult:
        async_result = []
