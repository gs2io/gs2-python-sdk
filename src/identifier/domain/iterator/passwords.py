...
from identifier import Gs2IdentifierRestClient, Password, request
from identifier.domain.cache.password import PasswordDomainCache
from core.domain.access_token import AccessToken


class DescribePasswordsIterator:
    ...
    _last: bool
    _result: List[Password]

    fetch_size: Optional[int]

    def __init__(
        self,
        password_cache: PasswordDomainCache,
        client: Gs2IdentifierRestClient,
        user_name: str,
    ):
        self._password_cache = password_cache
        self._client = client
    ...
    def _load(
        ...
        r = self._client.describe_passwords(
            request.DescribePasswordsRequest()
                .with_user_name(self._user_name)
                .with_page_token(self._page_token)
                .with_limit(self.fetch_size)
        )
        for item in r.items:
            self._password_cache.update(item)
