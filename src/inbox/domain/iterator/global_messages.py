...
from inbox import Gs2InboxRestClient, GlobalMessage, request
from inbox.domain.cache.global_message import GlobalMessageDomainCache
from core.domain.access_token import AccessToken


class DescribeGlobalMessagesIterator:
    ...
    def _load(
        ...
        r = self._client.describe_global_messages(
            request.DescribeGlobalMessagesRequest()
                .with_namespace_name(self._namespace_name)
        )
        for item in r.items:
            self._global_message_cache.update(item)
    ...
    def __next__(self) -> GlobalMessage:
