...
from mission import Gs2MissionRestClient, MissionTaskModelMaster, request
from mission.domain.cache.mission_task_model_master import MissionTaskModelMasterDomainCache
from core.domain.access_token import AccessToken


class DescribeMissionTaskModelMastersIterator:
    ...
    _last: bool
    _result: List[MissionTaskModelMaster]
    ...
    def _load(
        ...
        r = self._client.describe_mission_task_model_masters(
            request.DescribeMissionTaskModelMastersRequest()
                .with_namespace_name(self._namespace_name)
                .with_mission_group_name(self._mission_group_name)
                .with_page_token(self._page_token)
                .with_limit(self.fetch_size)
        )
        for item in r.items:
            self._mission_task_model_master_cache.update(item)
    ...
    def __next__(self) -> MissionTaskModelMaster:
