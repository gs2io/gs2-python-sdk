...
from mission import Gs2MissionRestClient, MissionGroupModel, request
from mission.domain.cache.mission_group_model import MissionGroupModelDomainCache
from core.domain.access_token import AccessToken


class DescribeMissionGroupModelsIterator:
    ...
    _index: int
    _last: bool
    _result: List[MissionGroupModel]
    ...
    def _load(
        ...
        r = self._client.describe_mission_group_models(
            request.DescribeMissionGroupModelsRequest()
                .with_namespace_name(self._namespace_name)
        )
        for item in r.items:
            self._mission_group_model_cache.update(item)
    ...
    def __next__(self) -> MissionGroupModel:
