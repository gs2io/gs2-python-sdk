...
from inventory import Gs2InventoryRestClient, InventoryModel, request
from inventory.domain.cache.inventory_model import InventoryModelDomainCache
from core.domain.access_token import AccessToken


class DescribeInventoryModelsIterator:
    ...
    def _load(
        ...
        r = self._client.describe_inventory_models(
            request.DescribeInventoryModelsRequest()
                .with_namespace_name(self._namespace_name)
        )
        for item in r.items:
            self._inventory_model_cache.update(item)
    ...
    def __next__(self) -> InventoryModel:
