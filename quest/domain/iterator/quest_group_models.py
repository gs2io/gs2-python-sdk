...
from quest import Gs2QuestRestClient, QuestGroupModel, request
from quest.domain.cache.quest_group_model import QuestGroupModelDomainCache
from core.domain.access_token import AccessToken


class DescribeQuestGroupModelsIterator:
    ...
    def _load(
        ...
        r = self._client.describe_quest_group_models(
            request.DescribeQuestGroupModelsRequest()
                .with_namespace_name(self._namespace_name)
        )
        for item in r.items:
            self._quest_group_model_cache.update(item)
    ...
    def __next__(self) -> QuestGroupModel:
